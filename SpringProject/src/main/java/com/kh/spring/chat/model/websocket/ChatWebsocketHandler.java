...
public class ChatWebsocketHandler extends TextWebSocketHandler{
	...
	public void afterConnectionEstablished(WebSocketSession session) {
		...
		// WebSocketSession : 웹소켓에 접속하거나 요청한 클라이언트의 세션정보
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception{
		...
		// 웹소켓 연결이 종료되는 경우 session안에 저장되어 있던 클라이언트의 session정보를 삭제
	}
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception{
		...
		if(result > 0) {
			...
			for(WebSocketSession s : sessions) {
				...
				// 반복을 진행중인 WebSocketSession안에 담겨있는 방 번호 == 메세지 안에 담겨있는 방 번호가 일치하는 경우 메세지 뿌리기
				int chatRoomNo = (int)s.getAttributes().get("chatRoomNo");
				
				// 메세지에 담겨있는 채팅방 번호와 chatRoomNo일치하는지 비교
